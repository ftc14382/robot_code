...
public class GoldAlignDetector extends DogeCVDetector {
    ...
    public Mat process(Mat input) {
        ...
        // Consider putting the imwrite call in its own thread.
        if (takeSnapshot) {
            takeSnapshot = false;
            Date        now           = new Date();
            DateFormat  dateFormat    = new SimpleDateFormat("yyyymmddhhmmss");
            String      imageFilename = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES) + File.Separator() + dateFormat.format(now) + "-robocap.png";
            Imgcodecs.imwrite(imageFilename, displayMat);
            lastSnapshotTime = new Instant();
        }
    ...
    //
    // If it has been more than 3 seconds since the last call, set takeSnapshot to true
    // which will cause the process method to save the image.
    //
    public void saveSnapshot () {
        Instant now = new Instanat();
        if (lastSnapshotTime == null) {
            takeSnapshot = true;
        } else if (lastSnapshotTime.plus(3, ChronoUnit.SECONDS).isBefore(now)) {
            takeSnapshot = true;
        }
    }
